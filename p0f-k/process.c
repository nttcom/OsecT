...
/* Get microsec part of unix time. */
...
/* Convert MAC address to a human-readable form. */

u8* mac_addr_to_str(u8* m) {
...
void parse_packet(void* junk, const struct pcap_pkthdr* hdr, const u8* data) {
  ...
  if (link_type == DLT_EN10MB) {
    ...
    memcpy(pk.dmac, data, 6);
    memcpy(pk.smac, data+6, 6);
  ...
  if (link_off > 0) {
    ...
    data += link_off;
    packet_len -= link_off;
    if (link_off >= 2 && *(data-2) == 0x81 && *(data-1) == 0x00) {
      data += 4;
      packet_len -= 4;
    }
  }
...
static struct packet_flow* create_flow_from_syn(struct packet_data* pk) {
  ...
  memcpy(nf->cli_mac, pk->smac, 6);
  memcpy(nf->srv_mac, pk->dmac, 6);
...
void add_nat_score(u8 to_srv, struct packet_flow* f, u16 reason, u8 score) {
  ...
  if (over_5 > 2 || over_2 > 4 || over_1 > 6 || over_0 > 8) {
    ...
    start_observation("ip sharing", 2, to_srv, f, 0);
    ...
    start_observation("host change", 2, to_srv, f, 0);
...
#undef REAF
  ...
  add_observation_field("reason", rea[0] ? (rea + 1) : NULL, 0);

  OBSERVF(0, "raw_hits", "%u,%u,%u,%u", over_5, over_2, over_1, over_0);
