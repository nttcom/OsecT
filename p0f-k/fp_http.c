...
static void fingerprint_http(u8 to_srv, struct packet_flow* f) {
  ...
  start_observation(to_srv ? "http request" : "http response", 4, to_srv, f, 0);

  if ((m = f->http_tmp.matched)) {

    OBSERVF(0, (m->class_id < 0) ? "app" : "os", "%s%s%s",
            fp_os_names[m->name_id], m->flavor ? " " : "",
            m->flavor ? m->flavor : (u8*)"");

  } else add_observation_field("app", NULL, 0);
  ...
  if (f->http_tmp.lang && isalpha(f->http_tmp.lang[0]) &&
    ...
    if (!languages[lh][pos]) add_observation_field("lang", NULL, 0);
      else add_observation_field("lang", 
           (lang = (u8*)languages[lh][pos + 1]), 0);

  } else add_observation_field("lang", (u8*)"none", 0);

  add_observation_field("params", dump_flags(&f->http_tmp, m), 0);

  add_observation_field("raw_sig", dump_sig(to_srv, &f->http_tmp), 0);
