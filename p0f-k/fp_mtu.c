...
void fingerprint_mtu(u8 to_srv, struct packet_data* pk, struct packet_flow* f) {
  ...
  start_observation("mtu", 2, to_srv, f, 0);
  ...
  if (i == sig_cnt[bucket]) add_observation_field("link", NULL, 0);
  else {

    add_observation_field("link", sigs[bucket][i].name, 0);
