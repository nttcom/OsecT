...
"  -d        - fork into background (requires -o, -O or -s)\n"
"\n"
...
void start_observation(char* keyword, u8 field_cnt, u8 to_srv,
                       struct packet_flow* f, u8 oslog_tcp_type) {

  if (obs_fields) FATAL("Premature end of observation.");

  if (!daemon_mode) {

    SAYF(".-[ %s(%s)/%u -> ", addr_to_str(f->client->addr, f->client->ip_ver),
         mac_addr_to_str(f->cli_mac),
         f->cli_port);
    SAYF("%s(%s)/%u (%s) ]-\n|\n", addr_to_str(f->server->addr, f->client->ip_ver),
         mac_addr_to_str(f->srv_mac),
         f->srv_port, keyword);
  ...
  if (log_file) {
      ...
      LOGF("[%s] mod=%s|cli=%s/%s/%u|",tmp, keyword, mac_addr_to_str(f->cli_mac), addr_to_str(f->client->addr,
