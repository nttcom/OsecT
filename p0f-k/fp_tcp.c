...
struct tcp_sig* fingerprint_tcp(u8 to_srv, struct packet_data* pk,
  ...
  if (to_srv) 
    start_observation(f->sendsyn ? "sendsyn probe" : "syn", 4, 1, f, f->sendsyn ? 0 : 1);
  else
    start_observation(f->sendsyn ? "sendsyn response" : "syn+ack", 4, 0, f, f->sendsyn ? 0 : 2);
...
void check_ts_tcp(u8 to_srv, struct packet_data* pk, struct packet_flow* f) {
  ...
  start_observation("uptime", 2, to_srv, f, 0);
  ...
  OBSERVF(0, "uptime", "%u days %u hrs %u min (modulo %u days)",
          (up_min / 60 / 24), (up_min / 60) % 24, up_min % 60,
          up_mod_days);

  OBSERVF(0, "raw_freq", "%.02f Hz", ffreq);
