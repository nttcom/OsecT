...
1. パケットキャプチャ: 監視用ネットワークインタフェースにおいてパケットをキャプチャし、PCAPファイルとして保存します。
2. 通信ログ生成: 1のPCAPファイルからクラウドでの分析用の通信ログを生成します。
3. 通信ログ送信: 2の通信ログをクラウドに送信します。
4. サポートログ生成: センサーPCのトラブルルシュートに使用するためのサポートログを生成します。
5. サポートログ送信: 4のサポートログをクラウドに送信します。
6. ファイル削除: 生成から一定期間経過した1のPCAPファイル及び2の通信ログを削除します。
...
概要: BACnet/IPパケット情報を記録

対象: BACnet/IPパケット

生成方法: オープンソースソフトウェアtsharkにより生成したファイルを元にNTT Com開発のソフトウェアにより生成

ファイル名: bacnet_service.log
...
| resp_p | 宛先ポート番号 |
| cmd | BACnetサービス |
| apdutype | BACnet APDUタイプ |
| objectype | BACnet オブジェクトタイプ |
...
2021年7月1日現在、サポートログの生成は、コマンド実行時にのみ行われます。クラウドへの送信は今後開発予定です。

## 4. ログ送信におけるセキュリティ対策

通信ログ・サポートログのクラウドへのアップロード時のセキュリティ対策として、以下を実施します。

- TLSによる暗号化
- 公開鍵証明書によるサーバ認証
- Google Cloud IAPによるクライアント認証

上記に加えて、NTT ComのUNO(Arcstar Universal One)およびFIC(Flexible InterConnect)を用いて、閉域ネットワーク経由でデータアップロードすることも可能です。
