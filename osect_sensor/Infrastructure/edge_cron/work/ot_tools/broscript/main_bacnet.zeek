...
event bacnet(c:connection, is_orig:bool,
    ...
    if(!c?$bacnet) {
	if (is_orig){
 	    c$bacnet = [$ts=network_time(), $id=c$id, $src=c$id$orig_h, $dst=c$id$resp_h, $resp_p=c$id$resp_p, $object_type="None"];
	}else{
  	    c$bacnet = [$ts=network_time(), $id=c$id, $src=c$id$resp_h, $dst=c$id$orig_h, $resp_p=c$id$orig_p, $object_type="None"];

	}
        #c$bacnet = [$ts=network_time(), $id=c$id, $src=c$id$resp_h, $dst=c$id$orig_h, $resp_p=c$id$resp_p, $object_type="None"];
	#c$bacnet = [$ts=network_time(), $id=c$id, $src=c$id$orig_h, $dst=c$id$resp_h, $resp_p=c$id$resp_p];
	}
    ...
    switch(bvlc_function) {
	    ...
	    c$bacnet$apdu_type += " (";
            c$bacnet$apdu_type += fmt("%d",apduType);
            c$bacnet$apdu_type += ")";
		    ...
		    c$bacnet$service_choice += " (";
            	    c$bacnet$service_choice += fmt("%d",serviceChoice);
            	    c$bacnet$service_choice += ")";
			    ...
			    if (serviceChoice == 0x07 && apduType == 1) {
       			    ...
       			    c$bacnet$object_type = object_types[object_type];
