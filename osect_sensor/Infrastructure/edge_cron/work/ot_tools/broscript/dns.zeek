...
export {
	...
	if ( idx?$trans_id ){
		info_insert$trans_id = idx$trans_id;
	}
	if ( idx?$rtt ){
		info_insert$rtt = idx$rtt;
	}
	if ( idx?$query ){
		info_insert$query = idx$query;
	}
	if ( idx?$qclass ){
		info_insert$qclass = idx$qclass;
	}
	if ( idx?$qclass_name ){
		info_insert$qclass_name = idx$qclass_name;
	}
	if ( idx?$qtype ){
		info_insert$qtype = idx$qtype;
	}
	if ( idx?$qtype_name ){
		info_insert$qtype_name = idx$qtype_name;
	}
	if ( idx?$rcode ){
		info_insert$rcode = idx$rcode;
	}
	if ( idx?$rcode_name ){
		info_insert$rcode_name = idx$rcode_name;
	}
	if ( idx?$AA ){
		info_insert$AA = idx$AA;
	}
	if ( idx?$TC ){
		info_insert$TC = idx$TC;
	}
	if ( idx?$RD ){
		info_insert$RD = idx$RD;
	}
	if ( idx?$RA ){
		info_insert$RA = idx$RA;
	}
	if ( idx?$Z ){
		info_insert$Z = idx$Z;
	}
	if ( idx?$answers ){
		info_insert$answers = idx$answers;
	}
	if ( idx?$TTLs ){
		info_insert$TTLs = idx$TTLs;
	}
	if ( idx?$rejected ){
		info_insert$rejected = idx$rejected;
	}
...
function create_aggregationData(info: Info): AggregationData
	...
	if ( info?$trans_id ){
		aggregationData$trans_id = info$trans_id;
	}
	if ( info?$rtt ){
		aggregationData$rtt = info$rtt;
	}
	if ( info?$query ){
		aggregationData$query = info$query;
	}
	if ( info?$qclass ){
		aggregationData$qclass = info$qclass;
	}
	if ( info?$qclass_name ){
		aggregationData$qclass_name = info$qclass_name;
	}
	if ( info?$qtype ){
		aggregationData$qtype = info$qtype;
	}
	if ( info?$qtype_name ){
		aggregationData$qtype_name = info$qtype_name;
	}
	if ( info?$rcode ){
		aggregationData$rcode = info$rcode;
	}
	if ( info?$rcode_name ){
		aggregationData$rcode_name = info$rcode_name;
	}
	if ( info?$AA ){
		aggregationData$AA = info$AA;
	}
	if ( info?$TC ){
		aggregationData$TC = info$TC;
	}
	if ( info?$RD ){
		aggregationData$RD = info$RD;
	}
	if ( info?$RA ){
		aggregationData$RA = info$RA;
	}
	if ( info?$Z ){
		aggregationData$Z = info$Z;
	}
	if ( info?$answers ){
		aggregationData$answers = info$answers;
	}
	if ( info?$TTLs ){
		aggregationData$TTLs = info$TTLs;
	}
	if ( info?$rejected ){
		aggregationData$rejected = info$rejected;
	}
...
function log_unmatched_msgs_queue(q: Queue::Queue)
	...
	for ( i in infos )
		...
		aggregationData = create_aggregationData(infos[i]);
		insert_res_aggregationData(aggregationData, infos[i]);
		}
	}
...
event dns_end(c: connection, msg: dns_msg) &priority=-5
	...
	if ( c?$dns && c$dns$saw_reply && c$dns$saw_query )
		...
		aggregationData = create_aggregationData(c$dns);
		insert_res_aggregationData(aggregationData, c$dns);
		delete c$dns;
		}
	}
...
# 		}
# 		if ( i?$qclass ){
# 			info$qclass = i$qclass;
# 		}
# 		if ( i?$qclass_name ){
# 			info$qclass_name = i$qclass_name;
# 		}
# 		if ( i?$qtype ){
# 			info$qtype = i$qtype;
# 		}
# 		if ( i?$qtype_name ){
# 			info$qtype_name = i$qtype_name;
# 		}
# 		if ( i?$rcode ){
# 			info$rcode = i$rcode;
# 		}
# 		if ( i?$rcode_name ){
# 			info$rcode_name = i$rcode_name;
# 		}
# 		if ( i?$AA ){
# 			info$AA = i$AA;
# 		}
...
# 		}
# 		if ( i?$answers ){
# 			info$answers = i$answers;
# 		}
# 		if ( i?$TTLs ){
# 			info$TTLs = i$TTLs;
# 		}
# 		if ( i?$rejected ){
# 			info$rejected = i$rejected;
# 		}
